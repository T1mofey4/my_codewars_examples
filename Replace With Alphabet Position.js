// In this kata you are required to, given 
// a string, replace every letter with its 
...
function alphabetPosition(str) {
    const strToArr = str.toLowerCase().replace(/[^a-z]/g, '').split('');
    const alphabet = [...'abcdefghijklmnopqrstuvwxyz'];
    let buff = [];
    for(let i = 0; i <= strToArr.length - 1; i++) {
        let a = strToArr[i];
        buff.push(alphabet.indexOf(a) + 1);
    }
    return buff.join(' ')
}
